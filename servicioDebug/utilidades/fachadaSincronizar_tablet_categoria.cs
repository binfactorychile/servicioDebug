...
namespace utilidades
{
    ...
    static public class FachadaSincronizar_tablet_categoria
    {
        ...
        public static int guardar(Sincronizar_tablet_categoria objeto)
        {
            try
            {
                Query query = new Query("insert", "sincronizar_tablet_categoria");
                ...
                int Sincronizar_tablet_categoria_ID = 0;
                foreach (DataRow fila in dataset.Tables[0].Rows)
                {
                    Sincronizar_tablet_categoria_ID = Utils.cint(fila["LAST_INSERT_ID()"].ToString());

                }
                return Sincronizar_tablet_categoria_ID;
            }
            catch (Exception ex)
            {
                Utils.EscribeLog(ex);
        ...
        public static void actualizar(Sincronizar_tablet_categoria objeto)
        {
            try
            {
                Query query = new Query("update", "sincronizar_tablet_categoria");
        ...
        public static string getQueryGuardar(Sincronizar_tablet_categoria objeto)
        {
            ...
            {
                Query query = new Query("insert", "sincronizar_tablet_categoria");
                query.AddInsert(ST_Sincronizar_tablet_categoria.categoria_ID, objeto.fcategoria_ID);
                query.AddInsert(ST_Sincronizar_tablet_categoria.tablet_ID, objeto.ftablet_ID);
                return query.listo() + ";";
            }
            catch (Exception ex)
            {
